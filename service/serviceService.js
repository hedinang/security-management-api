...
const get = async (serviceId) => {
    let apiResponse = {}
    let result = await mongodb.Service.find({
        id: serviceId,
        status: { $nin: ['REMOVED'] }
    }).lean();
    if (result.length) {
...
const list = async (body) => {
    ...
    try {
        ...
        let result = await mongodb.Service.find(cleanSearch).limit(limit).skip((page - 1) * limit).sort({ _id: -1 }).lean();

        const total_items = await mongodb.Service.count(cleanSearch)
...
const add = async (body) => {
    ...
    try {
        if (body) {
            let result = await mongodb.Service.create(body);
...
const update = async (body, file) => {
    ...
    try {
        if (body.id) {
            let service = await mongodb.Service.find({ id: body.id }).lean();
            if (service.length) {


                let serviceByName = await mongodb.Service.find({ name: name, id: { $ne: body.id } }).lean();
                if (serviceByName.length) {
                    apiResponse.status = httpStatus.StatusCodes.BAD_REQUEST
                ...
                if (origin_url?.includes('https://music2023.s3')) {
                    ...
                    if (file) {
                        const param = {
                            Bucket: 'music2023',
                            Key: `image/service/${uuidv4()}.${file.mimetype.split('/')[1]}`,
                            Body: file.buffer
                        }
                ...
                await mongodb.Service.findOneAndUpdate({ id: body.id }, { ...data, name }, { new: true, session });
                apiResponse.status = httpStatus.StatusCodes.OK
...
const removeById = async (serviceId) => {
    let apiResponse = {}
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        ...
        let result = await mongodb.Service.findOneAndUpdate({ id: serviceId }, { status: 'REMOVED' }, { new: true, session });
        apiResponse.data = result;
        apiResponse.status = httpStatus.StatusCodes.OK
        await session.commitTransaction();
    } catch (error) {
...
const remove = async (idList) => {
    ...
    try {
        ...
        let result = await mongodb.Service.updateMany({ id: { $in: idList } }, { $set: { status: 'REMOVED' } }, { new: true, session });
        apiResponse.data = result;
        apiResponse.status = httpStatus.StatusCodes.OK
        await session.commitTransaction();
    } catch (error) {
